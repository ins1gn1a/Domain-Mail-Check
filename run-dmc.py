...
items_xml.set('source','Domain Mail Check')
        ...
        for p in dmarc_params:
            ...
            if "ruf=" in p.lower():
                if "mailto:" not in p.lower():
                    ...
                    if "," in p:
                        vuln_review = vuln_review + vuln_append_print("\t[+] rua=: Mail failure reports will be sent to the following email addresses:")
                        dmarc_mail_list = p.split(",")
                        ...
                        vuln_review = vuln_review + vuln_append_print("\t[+] rua=" + p[5:] + ": Failure reports sent to this address.")
